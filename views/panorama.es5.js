 ...
 */
angular.module('otaniemi3dApp').controller('PanoramaCtrl', function ($scope, $stateParams, $window, $modal, sensorApi, $q, $interval) {
  ...
  var roomUrl = 'http://otaniemi3d.cs.hut.fi/omi/node/Objects/K1/' + self.roomId;
  ...
  getSensorData().then(displaySensors);
  ...
  function getSensorData() {
    ...
    return sensorApi.send('read', dataRequest).then(function (data) {
      return data;
    });
  }

  function displaySensors(sensors) {
    return getMetaData(sensors).then(waitForPanorama).then(makeSensorGroups).then(addSensorGroups);
  }
  ...
  function getMetaData(sensors) {
    ...
    return sensorApi.send('read', metaDataRequest).then(function (data) {
      self.sensors = data;
      return self.sensors;
    });
  }
  ...
  function makeSensorGroups(sensors) {
    return sensors.reduce(function (prev, curr) {
  ...
  function addSensorGroups(sensorGroups) {
    ...
    angular.forEach(sensorGroups, function (sensorGroup, key) {
      ...
      krpano.call('addsensor(' + [sensorGroup[0].id, pos[0], pos[1], sensorTooltip(sensorGroup)].join(',') + ',"' + JSON.stringify(sensorGroup) + '"' + ')');
    });
  ...
  function sendMetaData(sensors) {
    ...
    for (var i = 0; i < sensors.length; i++) {
      ...
      angular.forEach(sensors[i].metaData, function (value, key) {
        writeRequest.Objects.Object.Object.InfoItem[i].MetaData.InfoItem.push({
          '@name': key,
          'value': {
            'keyValue': value,
            '@type': 'xs:double'
  ...
  function sensorTooltip(sensors) {
    ...
    for (var i = 0; i < sensors.length; i++) {
      var sensorValue = sensors[i].values.length ? sensors[i].values[0].value : '';
      var sensorSuffix = sensors[i].suffix;
      sensorSuffix = sensorSuffix ? ' ' + sensorSuffix : '';

      sensorRows += ['[tr]', '[th]', sensors[i].name, '[/th]', '[td]', sensorValue, sensorSuffix, '[/td]', '[/tr]'].join('');
    }

    var sensorTable = ['[table class="tooltip-table"]', '[tr]', '[th colspan="2" style="text-align:center"]', self.roomId, '[/th]', '[/tr]', sensorRows, '[/table]'].join('');
