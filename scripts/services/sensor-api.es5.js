 ...
 */
angular.module('otaniemi3dApp').service('sensorApi', function ($http, $q, $interval, dataStorage, JXON) {
  ...
  this.parseObject = function (xml) {
    ...
    root.children('Object').each(function () {
      childObjects.push($(this).children('id').text());
    });

    root.children('InfoItem').each(function () {
      infoItems.push($(this).attr('name'));
    });
        ...
        if (name) {
          ...
          switch (name.toLowerCase()) {
            ...
            case 'pir':
              ...
              break;
            default:
              sensorName = name.charAt(0).toUpperCase() + name.slice(1);
          }
