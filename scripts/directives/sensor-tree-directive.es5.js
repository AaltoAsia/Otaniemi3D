 ...
 * # sensorTree
 */
angular.module('otaniemi3dApp').directive('sensorTree', function ($document, $http, sensorApi) {
  return {
    template: '<div></div>',
    restrict: 'E',
    scope: {
    ...
    link: function postLink(scope, element, attrs) {
      element.jstree({
        plugins: ['sort', scope.checkbox ? 'checkbox' : '', typeof attrs.search === 'string' ? 'search' : '', typeof attrs.dragSensor === 'string' ? 'dnd' : ''],
        search: {
            ...
            if (node.id === '#') {
              ...
              $http.get(node.original.url).success(function (data) {
                ...
                if (values.length) {
                  children = [{
                    values: values,
                    text: values[0].value + '  --  ' + values[0].time.toISOString(),
                    icon: false,
                    type: 'value'
                  }];
                }
