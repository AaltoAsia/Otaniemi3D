 ...
 */
angular.module('otaniemi3dApp').directive('heatMap', function (heatmapService, $q, $rootScope, sensorApi) {
  ...
  return {
    ...
    link: function link(scope, element) {
      ...
      getFloorplan(scope.floorplan).then(appendFloorplan).then(fetchSensorData).then(bindSensors).then(updateRoomColors);
      ...
      function appendFloorplan(floorplan) {
        ...
        svg = d3.select(svg).attr('width', '100%').attr('height', '100%').attr('pointer-events', 'all').attr('id', 'floorplan');
        ...
        zoomListener = d3.behavior.zoom().scaleExtent([0.5, 10]).scale(floorplan.scale).translate(floorplan.translate).on('zoom', function () {
          svg.select('g').attr('transform', ['translate(', d3.event.translate, ')', 'scale(', d3.event.scale, ')'].join(''));
      ...
      function fetchSensorData(floorplan) {
        ...
        d3.select(floorplan.svg).selectAll('[data-room-id]').each(function () {
        ...
        return sensorApi.send('read', sensorRequest).then(function success(data) {
          floorplan.data = data;
          return floorplan;
        }, function error() {
      ...
      function bindSensors(floorplan) {
        d3.select(floorplan.svg).selectAll('[data-room-id]').datum(function () {
        ...
        d3.select(floorplan.svg).selectAll('[data-room-id]').each(function () {
          ...
          for (var i = 0; i < data.sensors.length; i++) {
            if (data.sensors[i].type === scope.sensorType.name) {
              ...
              d3.select(this).style('fill', color.rgb).style('fill-opacity', color.opacity);
            }
          }
        });
      }
      ...
      function zoomAndPan(scale, translate) {
        if (zoomListener) {
          ...
          zoomListener.scale(scale).translate(translate).event(d3.select(element[0]).select('#floorplan'));
        }
      }
      ...
      scope.$on('room-selected', function (_, roomInfo) {
        d3.select(scope.floorplan.svg).selectAll('[data-room-id]').each(function () {
          var room = d3.select(this);

          //remove old room selection highlight
          room.style('stroke-width', null).style('stroke', null).style('stroke-opacity', null);

          if (room.datum().roomId === roomInfo.id) {
            room.style('stroke-width', '15').style('stroke', '#ffcc00').style('stroke-opacity', '0').transition().ease('cubic-in-out').style('stroke-opacity', '1');
